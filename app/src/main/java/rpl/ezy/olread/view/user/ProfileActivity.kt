...
//        val user_id = RequestBody.create(MediaType.parse("multipart/form-data"), sharedPreference!!.getIntSharedPreferences(USER_ID).toString())
        ...
        bt_done.setOnClickListener {
            editUser(
                ...
                email.text.toString(),
                sharedPreference!!.getIntSharedPreferences(USER_ID)
            )
            bt_done.visibility = View.GONE
            name.visibility = View.VISIBLE
            edit.visibility = View.GONE
    ...
    private fun editUser(username: String, email: String, user_id: Int) {
        val service =
        ...
        service.editUser(username, email, user_id).enqueue(object : Callback<ResponseSignup> {
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    Toast.makeText(this@ProfileActivity, "Berhasil Update!", Toast.LENGTH_SHORT)
                        .show()
                    sharedPreference!!.setSharedPreferences(ConstantUtils.USERNAME, username)
                    sharedPreference!!.setSharedPreferences(ConstantUtils.EMAIL, email)
                    getUser(intent.getIntExtra(USER_ID, 0))
                } else {
                    Log.d("LOGLOGAN", "${response.body()}")
                    Log.d("LOGLOGAN", "${response.errorBody()}")
                    Log.d("LOGLOGAN", "${response.isSuccessful}")
                    Log.d("LOGLOGAN", "${response.isSuccessful}")
                    Log.d("LOGLOGAN", response.message())
                }
            }
        })
    }
